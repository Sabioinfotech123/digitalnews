...
import { fetchLocalNewsById, importLocalNews } from '@/api/localNews'
...
import type { LocalNewsArticle } from '@/types/localNews'
...
function buildDefaultContent(article: LocalNewsArticle, description: string): string {
  const source = article.source_name || 'source'
  const body = description || article.title
  return [
    `<p>${body}</p>`,
    `<p><a href="${article.url}" target="_blank" rel="noopener noreferrer">Read original article</a></p>`,
    `<p><em>Imported from local news · ${source}</em></p>`,
  ].join('\n')
}

export function AdminLocalNewsDetailPage() {
  const { id } = useParams<{ id: string }>()
  const { message } = App.useApp()
  const navigate = useNavigate()
  const [form] = Form.useForm<LocalImportForm>()

  const [article, setArticle] = useState<LocalNewsArticle | null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (!active) return
        message.error(getApiErrorMessage(err, 'Could not load article'))
        setArticle(null)
      } finally {
  ...
  const handleValuesChange = (changed: Partial<LocalImportForm>, all: LocalImportForm) => {
  ...
  const handleFinish = async (values: LocalImportForm) => {
    if (!id) return
    setSaving(true)
    try {
      const result = await importLocalNews(id, {
  ...
  if (loading) {
    return <AppLoader tip="Loading article…" />
  }
    ...
    return (
      <div className="news-form-page">
        ...
        <p className="news-form-page__meta">
          Article not found or expired. Open it again from the Local news list.
        </p>
      </div>
    )
  }
      ...
      <div className="news-form-page__header">
        <div>
          ...
          <p className="news-form-page__meta" style={{ marginTop: '0.35rem', marginBottom: 0 }}>
            {article.source_name || 'Unknown source'} · {formatPublished(article.published_at)}
            {article.country ? ` · ${article.country}` : ''} · Local flag will be set
          </p>
        </div>
        <Space>
          <AppButton
