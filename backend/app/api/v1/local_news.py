...
def list_local_news(
    ...
    return LocalNewsService().list_articles(
        state=state,
        q=q,
        from_date=from_date,
        to_date=to_date,
...
def get_local_news_article(_admin: AdminUser, article_id: str) -> LocalNewsArticle:
    return LocalNewsService().get_article(article_id)
